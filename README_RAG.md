...
- `--min-size`: Минимальный размер чанка (символов)
- `--max-size`: Максимальный размер чанка (символов)
- `--overlap`: Перекрытие между чанками для контекста

**Стратегия chunking:**
- Разбиение по заголовкам Markdown (##, ###)
- Сохранение иерархии разделов
- Автоматическое разбиение больших разделов на параграфы
